...
-- Types, procedures and functions
 ...
 -- Printer Task
   ...
   type Traveler_Type is record
      ...
      Symbol: Character;
      Position: Position_Type;
      Direction: Direction_Type; -- New field for fixed direction
   end record;
   ...
   begin
      accept Init(Id : Integer; Seed : Integer; Symbol : Character) do
         ...
         Reset(G, Seed);
         Traveler.Position := (X => Id, Y => Id); -- Start on diagonal
         Steps := Min_Steps + Integer(Float(Max_Steps - Min_Steps) * Random(G));

         -- Assign a fixed direction based on ID
         if Id mod 2 = 0 then
      ...
      for Step in 1 .. Steps loop
         ...
         begin
            ...
            case Traveler.Direction is
               when Up => Move_Up(New_Pos);
               when Down => Move_Down(New_Pos);
               when Left => Move_Left(New_Pos);
               when Right => Move_Right(New_Pos);
               when others => null;
            end case;
