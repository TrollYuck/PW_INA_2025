...
procedure zad3_dekker is


-- Processes 

  Nr_Of_Processes : constant Integer := 2; -- Dekker's algorithm is for 2 processes
...
-- Dekker's Algorithm Shared Variables
  Want : array (0 .. Nr_Of_Processes - 1) of Boolean := (others => False);
  -- Want(I) is True if process I wants to enter the critical section.
  -- Corresponds to C(I+1) = 0 in the user's pseudocode (0 means want, 1 means not want).
  Turn : Integer range 0 .. Nr_Of_Processes - 1 := 0; 
...
-- Types, procedures and functions
  ...
  task body Process_Task_Type is
    ...
    Me    : Integer; -- Current process ID (0 or 1)
    Other : Integer; -- Other process ID (0 or 1)
  ...
  begin
    accept Init(Id: Integer; Seed: Integer; Symbol: Character) do
      ...
      Process.Id := Id;
      Me := Id;          -- Initialize Me
      Other := 1 - Me;   -- Initialize Other (since Nr_Of_Processes = 2)
...
--    for Step in 0 .. Nr_of_Steps loop
    for Step in 0 .. Nr_of_Steps/4 - 1  loop  -- TEST !!!
      ...
      Want(Me) := True;          -- I want to enter
      while Want(Other) loop     -- While the other process also wants to enter
         if Turn = Other then    -- If it's the other process's turn
            Want(Me) := False;   -- I retract my intention temporarily
            while Turn = Other loop -- Busy wait until it's my turn
               null; 
            end loop;
            Want(Me) := True;    -- Re-assert my intention, it's my turn now
         else -- Want(Other) is True and Turn = Me (it's my turn, but other also wants)
            -- Busy wait: I have the turn, but the other process also wants to enter.
            -- I must wait until the other process retracts its desire (Want(Other) becomes False).
            null; 
         end if;
      end loop;
      ...
      -- Dekker's Exit Protocol for Process `Me`
      Turn := Other;             -- Give turn to the other process
      Want(Me) := False;         -- I am no longer interested in the critical section
