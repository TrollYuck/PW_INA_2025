...
-- Peterson's Algorithm shared variables
  Interested : array (0 .. Nr_Of_Processes - 1) of Boolean := (others => False);
  pragma Volatile (Interested); -- Ensures reads/writes are not optimized out in loops
  Victim     : Integer range 0 .. Nr_Of_Processes - 1 := 0; -- Initial value can be 0 or 1
  pragma Volatile (Victim);    -- Ensures reads/writes are not optimized out in loops
...
-- Types, procedures and functions
  ...
  begin
    accept Init(Id: Integer; Seed: Integer; Symbol: Character) do
      ...
      -- Initialize Me and Other based on Process.Id
      Me := Process.Id;
      if Me = 0 then
          Other := 1;
      else
    ...
    for Step in 0 .. Nr_of_Steps/4 - 1  loop  -- TEST !!!
      ...
      Victim := Me; 
      loop
        -- Wait if the other process is interested and this process is the victim
        exit when not Interested(Other) or Victim /= Me; 
        -- Small delay to yield CPU in busy-wait, optional but good practice
