...
-- Types, procedures and functions
  ...
  -- Protected object to track the overall maximum ticket

  protected Max_Ticket_Tracker is
    procedure Update_Overall_Max(Ticket_Value : Integer);
    function Get_Overall_Max return Integer;
  private
    Stored_Max_Ticket : Integer := 0;
  end Max_Ticket_Tracker;

  protected body Max_Ticket_Tracker is
    procedure Update_Overall_Max(Ticket_Value : Integer) is
    begin
      if Ticket_Value > Stored_Max_Ticket then
        Stored_Max_Ticket := Ticket_Value;
      end if;
    end Update_Overall_Max;

    function Get_Overall_Max return Integer is
    begin
      return Stored_Max_Ticket;
    end Get_Overall_Max;
  end Max_Ticket_Tracker;
  ...
  begin
    ...
    Put_Line("MAX_TICKET=" & Integer'Image(Max_Ticket_Tracker.Get_Overall_Max) & ";"); -- Place labels with extra info here (e.g. "MAX_TICKET=...;" for Backery). 

  end Printer;

  -- Helper Max function
  Choosing: array(0 .. Nr_Of_Processes - 1) of Integer range 0..1
                                      := (others => 0);
...
--    for Step in 0 .. Nr_of_Steps loop
    for Step in 0 .. Nr_of_Steps/4 - 1  loop  -- TEST !!!
      ...
      declare
        New_Ticket : Integer := 1 + Max; -- Calculate new ticket
      begin
        Number(Process.Id) := New_Ticket; -- Assign ticket
        if New_Ticket > My_Highest_Ticket then -- Update local highest ticket
          My_Highest_Ticket := New_Ticket;
        end if;
      end;
      ...
      for J in 0 .. Nr_Of_Processes - 1 loop
        if J /= Process.Id then
          ...
          loop exit when
            (Number(J)=0) or (Number(Process.Id)<Number(J)) or (Number(Process.Id)=Number(J) and Process.Id<J);
          end loop;
        end if;
      end loop;
    ...
    Max_Ticket_Tracker.Update_Overall_Max(My_Highest_Ticket); -- report Max ticket
