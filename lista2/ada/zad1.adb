...
-- Types, procedures and functions
 ...
 -- Printer Task
   ...
   begin
      loop
         select
            accept Request (O: out Boolean) do
               O := not Occupied;
            end Request;
         or 
            accept Occupy do
   ...
   begin
      accept Init(Id : Integer; Seed : Integer; Symbol : Character) do
         ...
         loop
            ...
            Cell_Tasks(Traveler.Position.X, Traveler.Position.Y).Request(Success);
            if Success then 
               exit;
            else 
               delay 0.001;
      ...
      for Step in 1 .. Steps loop
         ...
         begin
            ...
            loop
               Cell_Tasks(New_Pos.X, New_Pos.Y).Request(Success);
               if Success then 
                  Cell_Tasks(Traveler.Position.X, Traveler.Position.Y).Free;
                  Cell_Tasks(New_Pos.X, New_Pos.Y).Occupy;
